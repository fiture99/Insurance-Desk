  ...
  if (compact) {
    return (
      <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
        {STAGES.map((s, i) => (
          <div key={s} title={s} style={{ width: 8, height: 8, borderRadius: 99, background: i <= idx ? STAGE_COLOR[stage] : "#E3E6EA" }} />
        ))}
      </div>
    );
  }
...
function Badge({ children, color = "#55606B", bg = "#F0F1F3" }: { children: React.ReactNode; color?: string; bg?: string }) {
  return (
    <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, color, background: bg, borderRadius: 5, padding: "3px 8px", letterSpacing: 0.2 }}>{children}</span>
  );
}
...
function StaffSignInModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  const [pin, setPin] = useState("");
  const [error, setError] = useState(false);
  function submit() { if (pin === STAFF_PIN) { onSuccess(); } else { setError(true); } }
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(27,42,65,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 20 }}>
      <div style={{ background: "#fff", borderRadius: 12, width: 340, maxWidth: "90vw", padding: 24 }}>
        ...
        <p style={{ fontSize: 13, color: "#55606B", marginTop: 8 }}>Enter the Insurance Desk PIN to log requests and update member status.</p>
        <input type="password" autoFocus value={pin} onChange={(e) => { setPin(e.target.value); setError(false); }} onKeyDown={(e) => e.key === "Enter" && submit()} placeholder="PIN" style={{ ...inputStyle, marginTop: 14, letterSpacing: 3, fontSize: 16, textAlign: "center" }} />
        {error && <div style={{ color: "#C1483F", fontSize: 12.5, marginTop: 8, fontWeight: 600 }}>Incorrect PIN. Try again.</div>}
        <button onClick={submit} style={{ marginTop: 16, width: "100%", border: "none", background: "#0E7A77", color: "#fff", borderRadius: 7, padding: "10px 14px", fontSize: 13.5, fontWeight: 700, cursor: "pointer" }}>Sign in</button>
      </div>
    </div>
  );
}
...
function RequestDetail({ r, editMode, onBack, onAdvance }: { r: Request; editMode: boolean; onBack: () => void; onAdvance: () => void }) {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 16 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Field label="Member ID" style={{ flex: 1, position: "relative" }}>
              <input value={form.memberId} onChange={handleIdChange} onBlur={() => setTimeout(() => setShowSuggestions(false), 150)} onFocus={() => { if (needsExistingMember && form.memberId.length > 0 && idSuggestions.length > 0) setShowSuggestions(true); }} style={{ ...inputStyle, borderColor: idMissing || idAlreadyExists ? "#C1483F" : "#D7DBE0" }} placeholder="e.g. 2025/B" />
              {showSuggestions && (
          ...
          {idAlreadyExists && <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#C1483F", fontSize: 12, fontWeight: 600, marginTop: -4 }}><AlertCircle size={12} />Member ID {form.memberId} already exists ({matchedMember!.name}). Use a new ID for an addition.</div>}
          <Field label="Reason">
    ...
    setRequests((prev) =>
      prev.map((r) => {
        ...
        if (next === "Actioned" && r.type === "Remove dependent") {
          setMembers((ms) => ms.map((m) => (m.id === r.memberId ? { ...m, status: "Inactive" } : m)));
        }
        if (next === "Actioned" && r.type === "Reinstate") {
          setMembers((ms) => ms.map((m) => (m.id === r.memberId ? { ...m, status: "Active" } : m)));
        }
        if (next === "Actioned" && r.type === "Add dependent") {
          setMembers((ms) => {
            if (ms.some((m) => m.id === r.memberId)) return ms;
            return [...ms, { id: r.memberId, name: r.memberName, company: r.company, role: "Dependent", status: "Active" }];
          });
        }
        return { ...r, stage: next };
      })
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", display: "flex", minHeight: "100vh", background: "#F6F7F9", color: "#1B2A41" }}>
      ...
      <div style={{ width: 220, background: "#1B2A41", color: "#fff", padding: "22px 16px", flexShrink: 0, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        ...
        <div style={{ marginTop: 30, display: "flex", flexDirection: "column", gap: 2 }}>
          {[{ key: "overview", label: "Overview", icon: LayoutGrid }, { key: "requests", label: "Requests", icon: FileText }, { key: "members", label: "Members", icon: Users }].map(({ key, label, icon: Icon }) => (
        ...
        <div style={{ marginTop: "auto", paddingTop: 24 }}>
          {editMode ? (
            <div style={{ padding: "10px 12px", background: "rgba(14,122,119,0.18)", borderRadius: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#6FD6D1", fontSize: 12, fontWeight: 700 }}><Unlock size={13} /> Editing unlocked</div>
              <button onClick={() => setEditMode(false)} style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 6, border: "none", background: "none", color: "#B8C1CE", fontSize: 11.5, fontWeight: 600, padding: 0 }}><LogOut size={12} /> Sign out</button>
            </div>
          ) : (
            <button onClick={() => setShowSignIn(true)} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "#B8C1CE", borderRadius: 8, padding: "9px 12px", fontSize: 12.5, fontWeight: 600 }}>
      ...
      <div style={{ flex: 1, padding: "26px 34px", maxWidth: 1100, overflowY: "auto" }}>
          ...
          <>
            ...
            <div style={{ marginTop: 30 }}>
              ...
              <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
                {requests.filter((r) => r.stage !== "Confirmed").length === 0 && <EmptyNote text="Nothing open. New instructions you log will show up here." />}
                {requests.filter((r) => r.stage !== "Confirmed").map((r) => (
        ...
        {tab === "requests" && !selected && (
          <>
            <TopBar title="Requests" subtitle="Instructions received from insurance providers by email or message." query={query} setQuery={setQuery} actionLabel={editMode ? "Log request" : null} onAction={() => setShowNewRequest(true)} />
            {!editMode && <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10, color: "#9AA2AB", fontSize: 12.5 }}><Lock size={12} /> View only — Insurance Desk staff can sign in to log or update requests.</div>}
        ...
        {tab === "requests" && selected && (
          <RequestDetail r={selected} editMode={editMode} onBack={() => setSelected(null)} onAdvance={() => advanceStage(selected.id)} />
        )}
      ...
      {showNewRequest && <NewRequestModal members={members} onClose={() => setShowNewRequest(false)} onSubmit={addRequest} />}
      {showSignIn && <StaffSignInModal onClose={() => setShowSignIn(false)} onSuccess={() => { setEditMode(true); setShowSignIn(false); }} />}
    </div>
  );
}
